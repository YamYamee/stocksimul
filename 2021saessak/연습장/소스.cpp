...
int cash; //현금 자산만
int isTerminate = 0;// 프로그램 종료여부

//메인 인터페이스. 현재 날짜의 경과, 보유한 자산, 보유한 주식, 수익등을 보여주고 주식 매수 매도 주식 차트를 볼 수 있음
void main_interface(); //주식을 구매하는 함수
void purchase(); //주식을 판매하는 함수
void sell(); //랜덤을 통해서 함수를 돌려서 주가 변동 - 한국의 기준에 맞춰서 상한가/하한가
void change_stock_price(); //이벤트를 통해서 주식의 개수 유상증자/무상증자
void change_stock_number(Stock arr[4][4]); //printf를 통해서 현재 나의 상황과 주식의 상황(모든 주식) 보여주는 함수
void pre_sit(); //주식의 전일 대비 증감과 현재 가격을 보여주는 함수
void show_stock(); //뉴스를 통해 랜덤 이벤트를 발생시키는 함수
void news(); //화면 전환을 담당하는 함수
...
void show_stock() {
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 4; j++) {
			printf("%s: 현재 주가: %d \n", arr[i][j].name, arr[i][j].price); printf("%s: 전일 대비: %d % \n", arr[i][j].name, arr_percent[i][j]);
		}
	}
}
